...
double external_potential(double val)
{
    ...
    return pow(val, 4) - 5 / 2.0 * pow(val, 2);
}
...
double variationalWaveFunction_second(double v)
{
    ...
    double mu = 0.786;
    double sigma = 0.622;
    return 1 / pow(sigma, 2) * (pow((v - mu) / sigma, 2) - 1) * exp(-pow(v - mu, 2) / (2 * pow(sigma, 2))) + 1 / pow(sigma, 2) * (pow((v + mu) / sigma, 2) - 1) * exp(-pow(v + mu, 2) / (2 * pow(sigma, 2)));
}
