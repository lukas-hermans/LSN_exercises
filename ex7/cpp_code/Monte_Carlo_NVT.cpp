...
void Input(void)
{
    ...
    // equilibration
    for (int istep = 0; istep < equi_step; istep++)
    {
        Move();
    }
...
// reset is used to delete existing files with instantaneous values at the beginning of the simulation
void Measure(int reset)
{
    double v = 0.0, w = 0.0;
    double vij, wij;
    double dx, dy, dz, dr;
    ...
    for (int i = 0; i < npart - 1; ++i)
    {
        for (int j = i + 1; j < npart; ++j)
        {
            ...
            for (int k = 0; k < nbins; k++)
            {
                ...
                {
                    walker[k + 2] += 2; // increment by 2 (see formula for g in lecture notes)
                }
            }
    ...
    // print instantaneous values for Epot and Pres
    if (print_inst == 1)
    {
        if (reset == 1)
        {
            ofstream Epot("../data/inst.epot.0");
            ofstream Pres("../data/inst.pres.0");

            Epot.close();
            Pres.close();
        }

        ofstream Epot("../data/inst.epot.0", ios::app);
        ofstream Pres("../data/inst.pres.0", ios::app);
...
void Averages(int iblk) //Print results for current block
{
    ...
    if (iblk == 1)
    {
        Epot.open("../data/output.epot.0");
        Pres.open("../data/output.pres.0");
        Gofr.open("../data/output.gofr.0");
        Gave.open("../data/output.gave.0");

        Epot << "M, epot (current block), epot, epot_error" << endl;
        Pres << "M, pres (current block), pres, pres_error" << endl;
    ...
    Pres.open("../data/output.pres.0", ios::app);
    ...
    Epot << iblk * nstep << ", " << stima_pot << ", " << glob_av[iv] / (double)iblk << ", " << err_pot << endl;
    //Pressure
    Pres << iblk * nstep << ", " << stima_pres << ", " << glob_av[iw] / (double)iblk << ", " << err_press << endl;

    //g(r)
    for (int i = 0; i < nbins; i++)
    {
        stima_g = blk_av[i + 2] / blk_norm / (rho * m_part * 4 * M_PI / 3 * (pow(i * bin_size + bin_size, 3) - pow(i * bin_size, 3)));
        glob_av[i + 2] += stima_g;
        glob_av2[i + 2] += stima_g * stima_g;
        ...
        if (iblk == nblk)
        {
            Gave << (2 * i + 1) / 2.0 * bin_size << ", " << glob_av[i + 2] / (double)iblk << ", " << err_gdir << endl;
        }
    }
